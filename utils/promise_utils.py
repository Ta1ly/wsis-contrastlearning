...
import numpy as np
import datasets
from datasets.image import apply_transform, adjust_transform_for_image, TransformParameters
from datasets.transform import transform_contour

# def get_box_from_mask_disturb(mask):
...
class PromiseDetection(datasets.PromiseDetection):
    def __init__(self, root, image_folder, gt_folder, transforms, margin=0, random_margin=True,
                 transform_generator=None, visual_effect_generator=None,
                 transform_parameters=TransformParameters()):
        super(PromiseDetection, self).__init__(root, image_folder, gt_folder, margin=margin, random_margin=random_margin)
...
def get_promise(root, image_folder, gt_folder, transforms, margin=0, random_margin=True,
                transform_generator=None, visual_effect_generator=None):
    ...
    # print('1', random_margin)
    dataset = PromiseDetection(root, image_folder, gt_folder, margin=margin, random_margin=random_margin,
