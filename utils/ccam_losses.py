...
def contrast_loss(fg_loss, bg_loss, neg_loss, fg_feats, bg_feats):
    fg_loss_v = fg_loss(fg_feats)
    bg_loss_v = bg_loss(bg_feats)
    neg_loss_v = neg_loss(bg_feats, fg_feats)
    return fg_loss_v + bg_loss_v + neg_loss_v
...
class ContrastLoss():
    def __init__(self, alpha=0.25, metric='cos', reduction='mean'):
        super(ContrastLoss, self).__init__()
        self.alpha = alpha
        self.metric = metric 
        self.reduction = reduction
        self.fg_loss = SimMaxLoss(alpha=alpha, metric=metric, reduction=reduction)
        self.bg_loss = SimMaxLoss(alpha=alpha, metric=metric, reduction=reduction)
        self.neg_loss = SimMinLoss(metric=metric, reduction=reduction)
    
    def __call__(self, fg_feats, bg_feats):
        loss = contrast_loss(self.fg_loss, self.bg_loss, self.neg_loss, fg_feats, bg_feats)
        return loss
