...
def bbox_prior_mask_generator(images, targets):
    # print(images)
    masks_shape = (images.shape[0],1,images.shape[2],images.shape[3])
    masks = images[0].new_full(masks_shape, 0)
    for target_index, target in enumerate(targets):
        boxes = target['boxes']
        for i in range(boxes.shape[0]):
            bbox = boxes[i].cpu().numpy().astype(np.int)
            
            # print(bbox)
            # print(masks.shape)
            masks[target_index,0,bbox[0]:bbox[2],bbox[1]:bbox[3]] = 1
    return masks
