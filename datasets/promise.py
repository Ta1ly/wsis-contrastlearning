...
class PromiseDetection(VisionDataset):
    ...
    def __init__(
        ...
        super(PromiseDetection, self).__init__(transform, target_transform, 
                                                transforms)
        self.categories = [{'name': 'prostate', 'id': 0}]         
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        # print(f'get item {index} {img_id} {self.image_names[index]}')
        ...
        for b in range(1, n_blob + 1):
            ...
            if self.margin > 0:
                if self.random_margin:
                    y1 -= randint(-1, self.margin)
                    x1 -= randint(-1, self.margin)
                    y2 += randint(-1, self.margin)
                    x2 += randint(-1, self.margin)
                    # print('random margin')
                else:
                    # print('fix margin')
                    y1 -= self.margin
